...
export default function Profile() {
  ...
  const handleSeeFollows = () => {
    history.replace(`/following/${id}`)
  }
  ...
  return (
    <div>
      ...
      >{`Profile of ${nome}`}</Typography>
      <Typography align="center">{`Member since ${formatDate(
        memberSince,
      )}`}</Typography>
      {(auth as UserInfo).id !== Number.parseInt(id) && (
      ...
      <div style={{ padding: 10 }}>
        ...
        >{`Total played games: ${totalGamesPlayed}`}</Typography>
        {gameRithms.map((gr) => {
          ...
          return (
            <Grid style={{ padding: 10 }} key={gr} container spacing={2}>
              ...
              <Grid item xs={3}>{`Won games: ${info.won} (${
                total ? (info.won / total) * 100 : 0
              }%)`}</Grid>
              <Grid item xs={3}>{`Drawn games: ${info.drawn} (${
                total ? (info.drawn / total) * 100 : 0
              }%)`}</Grid>
              <Grid item xs={3}>{`Lost games: ${info.lost} (${
                total ? (info.lost / total) * 100 : 0
              }%)`}</Grid>
            </Grid>
          )
        })}
        <Button
          style={{ width: '100%', fontSize: 20, color: 'purple' }}
          onClick={handleSeeGames}
        >
          See games
        </Button>
        <Button
          style={{ width: '100%', fontSize: 20, color: 'purple' }}
          onClick={handleSeeFollows}
        >
